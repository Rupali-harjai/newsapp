...
async componentDidMount(){
  console.log("cdm");
  let url =`https://newsapi.org/v2/top-headlines?country=in&apiKey=ee2137ba5c964c57bf01d5bd827afe1d&page=1&pageSize=${this.props.pageSize}`;
...
let url =`https://newsapi.org/v2/top-headlines?country=in&apiKey=ee2137ba5c964c57bf01d5bd827afe1d&page=${this.state.page - 1}&pageSize=${this.props.pageSize}`;
this.setState({loading:true});
  let data = await fetch(url);
  let parseData = await data.json()
  console.log(parseData);
      ...
      <div className='container my-3'>
        ...
        {!this.state.loading && this.state.articles.map((element) =>{
          return <div className='col-md-4' key={element.url}>
            <NewsItem title = {element.title ? element.title.slice(0,40) : ""} description ={element.description ? element.description.slice(0,80) : ""} imageUrl= {element.urlToImage ? element.urlToImage : "https://www.greatandhra.com/newphotos10/money21661196602.jpg"} newsUrl = {element.url}/>
            </div>
        })}
        </div>
